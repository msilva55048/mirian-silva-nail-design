-- Fase 7: mantém o claim no mesmo contrato de agenda pública.
-- Não altera dados existentes; apenas substitui a RPC para novos claims.
...
declare
  ...
  v_service public.services%rowtype; v_profile public.client_profiles%rowtype; v_id uuid; v_now timestamptz := now(); v_agenda_duration integer;
begin
  ...
  v_agenda_duration := case when btrim(v_service.name) = 'Reparo de Unha (Unitário)' then 30 else v_o.duration_minutes end;
  ...
  if exists (select 1 from public.schedule_blocks b where b.block_date=v_o.appointment_date and b.start_time < v_o.start_time + make_interval(mins=>v_agenda_duration) and b.end_time > v_o.start_time) then return jsonb_build_object('result','unavailable'); end if;
  if exists (select 1 from public.appointments a where a.appointment_date=v_o.appointment_date and coalesce(a.status,'') not in ('cancelled','canceled','cancelado','no_show','no-show') and a.start_time < v_o.start_time + make_interval(mins=>v_agenda_duration) and v_o.start_time < a.start_time + make_interval(mins=>case when btrim(coalesce(a.service_name,''))='Reparo de Unha (Unitário)' then 30 else coalesce(a.duration_minutes,0) end)) then return jsonb_build_object('result','unavailable'); end if;
  ...
  values(v_client,v_profile.full_name,v_profile.phone,v_profile.email,v_service.name,v_o.appointment_date,v_o.start_time,v_service.duration_minutes,v_service.price_cents,'confirmed') returning id into v_id;
