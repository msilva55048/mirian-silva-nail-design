-- An interrupted HTTP send may already have been accepted by Meta.
-- Hold it for reconciliation instead of automatically sending a duplicate.
...
begin
    ...
    set status = 'failed', failed_at = now(),
        error_message = 'Envio interrompido; conferir recebimento na Meta antes de autorizar nova tentativa.',
        updated_at = now()
    ...
    return query with candidates as (
        ...
        where wn.status = 'pending' and wn.attempts = 0
          ...
          and ((wn.notification_type = 'reminder_40h' and wn.scheduled_for >= now() - interval '6 hours')
            or (wn.notification_type = 'reminder_2h' and wn.scheduled_for >= now() - interval '1 hour'))
