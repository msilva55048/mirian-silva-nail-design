...
await context.route("**/*", async (route) => {
    ...
    if (resource === "waiting_list") {
        if (method === "POST") {
            const {client_id, preferred_date, preferred_time, preferred_dates, preferred_times} = request.postDataJSON();
            if (entries.some((entry) => entry.client_id === client_id)) return json({code: "23505", message: "duplicate"}, 409);
            entries.push({id: `w${++entryCounter}`, client_id, preferred_date, preferred_time, preferred_dates, preferred_times, created_at: `2030-01-07T06:00:0${entryCounter}Z`});
        }
        if (method === "DELETE") {
...
try {
    ...
    const add = async (name, date = "2030-01-07", time = "08:00") => {
        ...
        await page.getByLabel("Adicionar cliente à lista de espera").fill(name);
        await page.locator(".admin-waiting-list__results").getByRole("button").first().click();
        const calendar = page.getByRole("region", {name: "Datas de interesse", exact: true});
        ...
        for (const selected of await calendar.locator("button[aria-pressed=true]").all()) await selected.click();
        await calendar.getByRole("button", {name: date.split("-").reverse().join("/"), exact: true}).click();
        for (const selected of await page.getByRole("region", {name: "Horários de interesse", exact: true}).locator("button[aria-pressed=true]").all()) await selected.click();
        await page.getByRole("region", {name: "Horários de interesse", exact: true}).getByRole("button", {name: time, exact: true}).click();
    ...
    for (const width of [375, 390, 430, 1280]) await checkWidth(width);
    assert.match(await page.locator(".admin-waiting-list__entries").innerText(), /Datas de interesse: 07\/01/);
    assert.match(await page.locator(".admin-waiting-list__entries").innerText(), /Horários de interesse: 08:00/);
    await waitFor(() => Boolean(realtimeWaiting));
    entries[0].preferred_dates = ["2030-01-08"];
    realtimeWaiting(entries[0]);
    await page.getByText("Datas de interesse: 08/01", {exact: true}).waitFor();
    ...
    entries[0].preferred_dates = ["2030-01-07"];
    realtimeWaiting(entries[0]);
    await page.getByText("Datas de interesse: 08/01", {exact: true}).waitFor({state: "hidden"});
    await bookFirst();
    for (const width of [375, 390, 430, 1280]) await checkWidth(width);
    ...
    realtimeWaiting(entries[0]);
    await openView("Lista de espera");
    await page.getByText("Datas de interesse: Não informadas", {exact: true}).waitFor();
    await page.getByText("Horários de interesse: Não informados", {exact: true}).waitFor();
    await bookFirst();
    ...
    // Multiple independent sets, one persisted row, and no arbitrary booking choice.
    await page.getByLabel("Adicionar cliente à lista de espera").fill("Beatriz");
    await page.locator(".admin-waiting-list__results button").first().click();
    const multiCalendar = page.getByRole("region", {name: "Datas de interesse", exact: true});
    const selectDate = (date) => multiCalendar.getByRole("button", {name: date, exact: true}).click();
    assert(await page.getByRole("button", {name: "Adicionar à lista de espera", exact: true}).isDisabled());
    for (const date of ["12/01/2030", "08/01/2030", "10/01/2030", "09/01/2030", "07/01/2030"]) await selectDate(date);
    await selectDate("09/01/2030");
    await selectDate("09/01/2030");
    await multiCalendar.getByRole("button", {name: "Próximo mês"}).click();
    await selectDate("01/02/2030");
    await multiCalendar.getByRole("button", {name: "Mês anterior"}).click();
    assert.equal(await multiCalendar.locator("button[aria-pressed=true]").count(), 5);
    const multiTimes = page.getByRole("region", {name: "Horários de interesse", exact: true});
    for (const time of ["19:00", "11:00", "13:00"]) await multiTimes.getByRole("button", {name: time, exact: true}).click();
    await multiTimes.getByRole("button", {name: "13:00", exact: true}).click();
    assert.equal(await multiTimes.locator("button[aria-pressed=true]").count(), 2);
    await multiTimes.getByRole("button", {name: "13:00", exact: true}).click();
    for (const width of [375, 390, 430, 1280]) await checkWidth(width);
    const expectedDates = ["2030-01-07", "2030-01-08", "2030-01-09", "2030-01-10", "2030-01-12", "2030-02-01"];
    const expectedTimes = ["11:00", "13:00", "19:00"];
    const beforeMulti = {appointments: appointments.length, blocks: blocks.length, calls: rpcCalls};
    await page.getByRole("button", {name: "Adicionar à lista de espera", exact: true}).click();
    await waitFor(() => entries.length === 2);
    assert.deepEqual(entries[1].preferred_dates, expectedDates);
    assert.deepEqual(entries[1].preferred_times, expectedTimes);
    assert.equal(entries.filter((entry) => entry.client_id === "c4").length, 1);
    assert.deepEqual({appointments: appointments.length, blocks: blocks.length, calls: rpcCalls}, beforeMulti);
    await waitFor(async () => await page.getByLabel("Adicionar cliente à lista de espera").inputValue() === "");
    await page.reload();
    await openView("Lista de espera");
    const multiCard = page.locator(".admin-waiting-list__entries article").filter({hasText: "Beatriz"});
    await multiCard.waitFor();
    for (const date of expectedDates) assert((await multiCard.innerText()).includes(date.slice(5).split("-").reverse().join("/")));
    assert(!(await multiCard.innerText()).includes("/2030"));
    assert((await multiCard.innerText()).includes("11:00 · 13:00 · 19:00"));
    await waitFor(() => Boolean(realtimeWaiting));
    expectedTimes.push("20:00");
    entries[1].preferred_times = [...expectedTimes];
    realtimeWaiting(entries[1]);
    await multiCard.getByText("Horários de interesse: 11:00 · 13:00 · 19:00 · 20:00", {exact: true}).waitFor();
    for (const width of [375, 390, 430, 1280]) await checkWidth(width);
    await page.setViewportSize({width: 375, height: 844});
    await multiCard.screenshot({path: "node_modules/.cache/admin-features/multiple-card-375.png"});
    await multiCard.getByRole("button", {name: "Agendar", exact: true}).click();
    const summary = page.locator(".admin-manual-booking__summary");
    assert.match(await summary.innerText(), /Selecione o dia/);
    assert.match(await summary.innerText(), /Selecione o horário/);
    assert((await page.getByRole("region", {name: "Preferências da cliente"}).innerText()).includes("11:00 · 13:00 · 19:00"));
    assert((await page.getByRole("region", {name: "Preferências da cliente"}).innerText()).includes("07/01 · 08/01 · 09/01 · 10/01 · 12/01 · 01/02"));
    assert.equal(await page.locator(".admin-manual-times button.is-selected").count(), 0);
    assert(await page.getByRole("button", {name: "Salvar agendamento", exact: true}).isDisabled());
    for (const width of [375, 390, 430, 1280]) await checkWidth(width);
    await page.locator(".admin-manual-month-calendar__grid").getByRole("button", {name: "7", exact: true}).click();
    assert.equal(await page.locator(".admin-manual-times").getByRole("button", {name: "11:00", exact: true}).count(), 0);
    assert.equal(await page.locator(".admin-manual-times").getByRole("button", {name: "09:00", exact: true}).count(), 0);
    await page.locator(".admin-manual-times").getByRole("button", {name: "13:00", exact: true}).count().then((count) => assert.equal(count, 0));
    await page.locator(".admin-manual-times").getByRole("button", {name: "19:00", exact: true}).click();
    failBooking = true;
    await page.getByRole("button", {name: "Salvar agendamento", exact: true}).click();
    await page.getByText("Falha simulada", {exact: false}).waitFor();
    assert.deepEqual(entries[1].preferred_dates, expectedDates);
    assert.deepEqual(entries[1].preferred_times, expectedTimes);
    await page.getByRole("button", {name: "Cancelar", exact: true}).click();
    assert.equal(entries.length, 2);
    ...
    await page.getByRole("button", {name: "Salvar agendamento", exact: true}).click();
    await waitFor(() => entries.length === 1);
    console.log("PASS: multi-date/month toggles, sorted independent sets, one row, reload, no reservation, explicit booking choice, failure/cancel preservation, success removal, four widths");
