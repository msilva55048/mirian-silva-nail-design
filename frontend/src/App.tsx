    ...
    return (
        <main className="home">
            ...
            {clientUserId && clientProfile ? (
                ...
                .client-notification-push-control__copy span { color: #806c74; font-size: .76rem; }
                .client-notification-push-toggle { position: relative; flex: 0 0 auto; width: 54px; height: 30px; border: 0; border-radius: 999px; padding: 0; background: #bd5364; cursor: pointer; transition: background .2s ease; }
                .client-notification-push-toggle.is-active { background: #4d9b68; }
                .client-notification-push-toggle:disabled { cursor: wait; opacity: .65; }
                .client-notification-push-toggle::after { content: ""; position: absolute; top: 4px; left: 4px; width: 22px; height: 22px; border-radius: 50%; background: #fff; box-shadow: 0 1px 3px rgba(0,0,0,.2); transition: transform .2s ease; }
                .client-notification-push-toggle.is-active::after { transform: translateX(24px); }
...
type AdminSwapStep = "first" | "second" | "confirm";
...
const adminSevenDaysOnlyStyles = `
/* Apenas ADM > Agendamentos */
.admin-agenda-date-picker__week-days--seven {
...
.admin-swap-confirm-card p { margin: 0; }

@media (max-width: 520px) {
    .admin-swap-active-card { align-items: flex-start; flex-direction: column; }
}
...
function AdminPanel() {
    ...
    const [swapStep, setSwapStep] = useState<AdminSwapStep | null>(null);
    const [swapFirstClient, setSwapFirstClient] = useState<AdminClient | null>(null);
    ...
    function manualAppointmentConflicts(
        ...
        if (hasScheduleBlockConflict(adminBlocks, date, time, selectedServiceDurationMinutes)) return true;
    ...
    function getSwappableAppointments(client: AdminClient) {
        return client.appointments
            .filter((item) => (item.status === "pending" || item.status === "confirmed") && getAppointmentDateTime(item).getTime() > Date.now())
            .sort((a, b) => getAppointmentDateTime(a).getTime() - getAppointmentDateTime(b).getTime());
    }

    function beginAppointmentSwap(client: AdminClient) {
        setSwapError("");
        setSwapFirstClient(client);
        setSwapSecondClient(null);
    ...
    function chooseSecondSwapAppointment(appointment: AdminAppointment, client: AdminClient) {
        setSwapSecondClient(client);
        setSwapSecondAppointment(appointment);
        setSwapStep("confirm");
    }

    function closeAppointmentSwap() {
        if (isSwapping) return;
        setSwapStep(null);
        setSwapFirstClient(null);
        setSwapSecondClient(null);
        setSwapFirstAppointment(null);
        setSwapSecondAppointment(null);
        setSwapError("");
    ...
    async function confirmAppointmentSwap() {
        if (!swapFirstAppointment || !swapSecondAppointment || isSwapping) return;
        setIsSwapping(true);
        setSwapError("");
        const {data, error} = await supabase.rpc("admin_swap_appointments", {
            p_first_appointment_id: swapFirstAppointment.id,
            p_second_appointment_id: swapSecondAppointment.id,
        });
        if (error) {
            setSwapError(error.message || "Não foi possível trocar os horários.");
            setIsSwapping(false);
            return;
        }
        const result = data as {first_new_date?: string; first_new_time?: string; second_new_date?: string; second_new_time?: string} | null;
        setAppointments((current) => current.map((item) => {
            if (item.id === swapFirstAppointment.id) return {...item, appointment_date: result?.first_new_date ?? swapSecondAppointment.appointment_date, start_time: result?.first_new_time ?? swapSecondAppointment.start_time};
            if (item.id === swapSecondAppointment.id) return {...item, appointment_date: result?.second_new_date ?? swapFirstAppointment.appointment_date, start_time: result?.second_new_time ?? swapFirstAppointment.start_time};
            return item;
        }));
        setPanelSuccess("Horários trocados com sucesso.");
        setIsSwapping(false);
        closeAppointmentSwap();
    }
    ...
    const manualAvailableTimes = useMemo(() => {
        ...
        return candidateStarts
            .filter((candidateStart) => {
                ...
                const conflictsWithBlock = hasScheduleBlockConflict(
                    adminBlocks, manualDate, minutesToTime(candidateStart), selectedServiceDurationMinutes,
                );
                return !isPastToday && !conflictsWithAppointment && !conflictsWithBlock;
            })
            .map(minutesToTime);
    }, [
        manualDate,
            ...
            <section className="admin-panel">
                ...
                {showAdminNotifications && <div className="client-modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) closeAdminNotifications(); }}>
                    <section className="client-modal client-notifications-modal admin-notifications-modal" role="dialog" aria-modal="true" aria-labelledby="admin-notifications-title">
                        ...
                        <section className="referral-state-card"><span className="client-modal__eyebrow">Indicações</span><h3>Acompanhe as indicações das clientes.</h3>{isLoadingAdminReferrals ? <p>Carregando indicações...</p> : adminReferralsError ? <p role="alert">{adminReferralsError}</p> : adminReferrals.length ? Object.values(adminReferrals.reduce<Record<string, {name: string; items: AdminReferral[]}>>((groups, referral) => { const group = groups[referral.referrer_id] ?? {name: referral.referrer_name, items: []}; group.items.push(referral); groups[referral.referrer_id] = group; return groups; }, {})).map((group) => <div key={group.name} className="referral-state-card__group"><strong>{group.name}</strong>{group.items.map((referral) => <article key={referral.referral_id} className="referral-state-card__item"><span>{referral.referred_name}</span><span className={`referral-state-badge referral-state-badge--${referral.referral_status.toLowerCase()}`}>{referral.referral_status}</span>{referral.referral_status === "REALIZADA" && <small>Desconto de {referral.discount_percent ?? 30}% {referral.reward_status === "reserved" ? "reservado" : "disponível"}</small>}</article>)}</div>) : <p>Nenhuma cliente possui indicação ativa no momento.</p>}</section>
                ...
                <div className="admin-dashboard-cards">
                    ...
                    <button className={`admin-dashboard-card${adminView === "waiting" ? " is-active" : ""}`} type="button" onClick={() => openAdminDashboardView("waiting")}><strong>Lista de espera</strong><span>Clientes aguardando uma vaga.</span></button>
                ...
                {(adminView === "agenda" || adminView === "week") && (
                    <section className="admin-top-agenda">
                                                ...
                                                {showAgendaMonthCalendar && (
                                                    <div className="admin-manual-month-calendar admin-month-calendar--same-size">
                                                        <div className="admin-manual-month-calendar__header">
                                                            ...
                                                            <strong>
                                                                {agendaCalendarMonth.toLocaleDateString("pt-BR", {
                                                                    month: "long",
                                                                    year: "numeric",
                                                                })}
                                                            </strong>
                                                        ...
                                                        <div className="admin-manual-month-calendar__grid">
                                                            {getAgendaMonthCalendarCells().map((date, index) => {
                                                                if (!date) {
                                                                    return (
                                                                        <span
                                                                            className="is-empty"
                        ...
                        {adminView === "agenda" && (
                            <div className="admin-clients__search admin-appointments__search">
                                <label>
                                    Buscar cliente
                                    <input
                ...
                {adminView === "month" ? (
                    ...
                    <section className="admin-clients">
                        ...
                        {swapFirstAppointment && swapFirstClient && (
                            <div className="admin-panel-success admin-swap-active-card" role="status">
                        ...
                        <section className={`admin-new-appointment${adminView === "new" ? " admin-new-appointment--new" : ""}`}>
                            <div className="admin-new-appointment__header">
                                <div>
                                    ...
                                    <p>{adminView === "waiting" ? "Clientes aguardando uma vaga, da mais antiga à mais recente." : "Busque a cliente cadastrada e escolha serviço, dia e horário pela agenda exclusiva do painel ADM."}</p>
                                </div>
                                <button
                                    className="admin-new-appointment__toggle"
                                    type="button"
                                    hidden={adminView === "waiting"}
                        ...
                        <section className="admin-modal" role="dialog" aria-modal="true" aria-labelledby="swap-appointments-title">
                            ...
                            <div className="admin-modal__body">
                                {swapStep === "first" && getSwappableAppointments(swapFirstClient).map((appointment) => (
                                    <button key={appointment.id} type="button" className="admin-dashboard-card admin-swap-card" onClick={() => chooseFirstSwapAppointment(appointment)}>
                                        <strong>{formatAdminDate(appointment.appointment_date)} às {String(appointment.start_time).slice(0, 5)}</strong><span>{appointment.service_name} · {getAppointmentStatusLabel(appointment.status)}</span>
                                    </button>
                                ))}
                                ...
                                {swapStep === "second" && swapSecondClient && <div className="admin-modal__body"><p>Agendamentos de {swapSecondClient.name}:</p>{getSwappableAppointments(swapSecondClient).map((appointment) => <button key={appointment.id} type="button" className="admin-dashboard-card admin-swap-card" onClick={() => chooseSecondSwapAppointment(appointment, swapSecondClient)}><strong>{formatAdminDate(appointment.appointment_date)} às {String(appointment.start_time).slice(0, 5)}</strong><span>{appointment.service_name}</span></button>)}</div>}
                                {swapStep === "confirm" && swapSecondClient && swapFirstAppointment && swapSecondAppointment && <>
                                    <div className="admin-swap-confirm-card"><strong>Antes</strong><p>{swapFirstClient.name}: {formatAdminDate(swapFirstAppointment.appointment_date)} às {String(swapFirstAppointment.start_time).slice(0, 5)} · {swapFirstAppointment.service_name}</p><p>{swapSecondClient.name}: {formatAdminDate(swapSecondAppointment.appointment_date)} às {String(swapSecondAppointment.start_time).slice(0, 5)} · {swapSecondAppointment.service_name}</p></div>
                                    <div className="admin-swap-confirm-card"><strong>Depois</strong><p>{swapFirstClient.name}: {formatAdminDate(swapSecondAppointment.appointment_date)} às {String(swapSecondAppointment.start_time).slice(0, 5)} · {swapFirstAppointment.service_name}</p><p>{swapSecondClient.name}: {formatAdminDate(swapFirstAppointment.appointment_date)} às {String(swapFirstAppointment.start_time).slice(0, 5)} · {swapSecondAppointment.service_name}</p></div>
