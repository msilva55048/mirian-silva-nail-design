...
function PublicSite() {
    ...
    const isLoyaltyPreview = new URLSearchParams(typeof window !== "undefined" ? window.location.search : "").get("loyaltyPreview") === "1";
    const loyaltyDisplayCard: LoyaltyCard | null = isLoyaltyPreview ? {
        prelaunch: false,
        stars: 7,
        reward_available: false,
        last_eligible_completed_at: "2026-10-01",
    ...
    return (
        <main className="home">
            ...
            {clientUserId && clientProfile ? (
                ...
                .client-loyalty-card button { border: 0; border-radius: 10px; padding: 6px 10px; background: var(--gradient-primary-action); color: #fff; font: inherit; font-size: .78rem; font-weight: 800; line-height: 1.1; white-space: nowrap; cursor: pointer; box-shadow: 0 6px 16px rgba(83,48,58,.16); transition: transform .16s ease, box-shadow .16s ease; }
                .client-loyalty-card button:hover, .client-loyalty-card button:focus-visible { background: var(--gradient-primary-action-hover); transform: translateY(-1px); box-shadow: 0 8px 20px rgba(83,48,58,.22); }
